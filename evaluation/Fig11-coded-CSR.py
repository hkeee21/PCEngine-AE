...
mapping_format_list = ['coded-csr', 'matrix']
dataset_col = []
model_col = []
operator_col = []
speedup_col = []

buffer = torch.rand((512 * 30000 * 27,), dtype=torch.float, device=device)
...
for d, dataset in enumerate(dataset_list):
    ...
    DataLoader = torch.utils.data.DataLoader(
        Dataset, 
        batch_size= 1 if dataset == 'kitti' else 4, 
    ...
    for model in model_list:
        ...
        for r in range(results_len):
            ...
            % (results_dict['coded-csr-gather'][r], results_dict['vanilla-gather'][r], 
                 (results_dict['vanilla-gather'][r]/results_dict['coded-csr-gather'][r])))
            print('scatter: %.4f - %.4f [%.4f]' 
            % (results_dict['coded-csr-scatter'][r], results_dict['vanilla-scatter'][r],
